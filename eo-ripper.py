...
def check_wordpress(email):
	try:
		...
		print(C.C.colores.alert + "|--[WARNING][LinkedIn][>] Error..." + C.C.colores.normal)
	...
	try:
		...
		for text in soup.findAll("h1"):
			...
			if "Sorry" in text or "Lo sentimos," in text:
				print("|--[INFO][Twitter][" + C.C.colores.blue+ username + C.C.colores.normal + "][>] Account doesn't exist...")
			else:
				print(C.C.colores.green + "|--[INFO][Twitter][" + C.colores.blue+ username + C.colores.green + "][>] The account exist." + C.colores.normal)
...
def attack(email):
	...
	if R.check_email(email):
		...
		try:
			...
			print("[INFO][TARGET][>] " + email)
			print(C.colores.alert + "|--[INFO][EMAIL][>] No verification possible... " + C.colores.normal)
			print("|--[INFO][>] have you installed all the libraries? pip3 install -r requeriments.txt")
